...
exports.markdown = function (version, commits, options) {
  ...
  var m = /(^|.*\D)(\d+)(\D*)/.exec(options.tag); // last number in string
  // next version tag
  version = m ? `${m[1]}${1 + Number(m[2])}${m[3]}` : version + '-next';
  ...
  if (version) {
    heading += ' [' + version.slice(1) + '](' + options.repoUrl + '/compare/' + options.tag + '...' + version + ') (' + date + ')';
  } else {
    heading += ' ' + date;
  }
  ...
  commits.sort((a, b) => a.subject.localeCompare(b.subject))
  ...
  .each(function (commit) {
    ...
    // allow filtering default other types
    if (options.exclude.includes(type)) return;
